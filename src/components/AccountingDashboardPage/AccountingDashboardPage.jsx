...
const AccountingDashboardPage = () => {
    ...
    const oneMonthAgo = new Date(new Date().setMonth(new Date().getMonth() - 1)).toISOString().split('T')[0]; // Default start date
     ...
     */
    const fetchAccountingData = useCallback(async (type = '', startDate = '', endDate = '') => {
        ...
        if (!token) {
            console.error("AccountingDashboardPage: No authentication token found. Redirecting to login.");
            clearAuthData();
            navigate('/');
            return;
        }
        ...
        try {
            ...
            const typeSummaryUrl = buildUrl(`${BACKEND_URL}/api/transactions/type-summary`, {
                startDate: startDate,
                endDate: endDate
            });
            ...
            if (!loansReceivableResponse.ok) {
                ...
                throw new Error(errorData.message || 'Failed to fetch loans receivable summary.');
            }
            ...
            if (!transactionsResponse.ok) {
                ...
                throw new Error(errorData.message || 'Failed to fetch transactions list.');
            }
            ...
            toast.error(`Error loading accounting data: ${err.message || "Network error"}`);
    ...
    }, [BACKEND_URL, navigate]); // Dependencies for useCallback: backend URL and navigate function
    ...
    if (loading) {
        return <div className="accountingDashboardContainer accountingLoading">Loading accounting data...</div>;
    }

    if (error) {
        return <div className="accountingDashboardContainer accountingErrorMessage">Error: {error}</div>;
    }
            ...
            <div className="accountingDashboardContent">
                ...
                <section className="summaryCardsGrid">
                    ...
                    <div className="summaryCard netCashFlow">
                        <h3>Net Cash Flow (Transactions)</h3> {/* Updated label for clarity */}
                        ...
                        <div className="typeSummaryGrid">
                            {Object.entries(typeSummary).map(([type, amount]) => (
                                <div key={type} className="typeSummaryCard">
                                    <h4>{type.charAt(0).toUpperCase() + type.slice(1)}</h4> {/* Capitalize type */}
                                    <p>ZMW {amount.toFixed(2)}</p>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <p>No transaction type summaries available for the selected period.</p>
                    )}
                </section>
                            ...
                            <table className="transactionsTable">
                                ...
                                <tbody>
                                    {transactions.map(t => (
                                        <tr key={t._id}>
                                            <td>{t.date}</td>
                                            <td><span className={`transactionTypeTag ${t.type}`}>{t.type.charAt(0).toUpperCase() + t.type.slice(1)}</span></td>
                                            <td>{t.amount.toFixed(2)}</td>
                                            <td><span className={`transactionDirectionTag ${t.direction}`}>{t.direction.charAt(0).toUpperCase() + t.direction.slice(1)}</span></td>
                                            <td><span className={`transactionStatusTag ${t.status?.toLowerCase()}`}>{t.status || 'N/A'}</span></td>
                                            <td>{t.clientName}</td>
                                            <td>{t.loan ? t.loan._id.substring(0, 8) + '...' : 'N/A'}</td>
                                            <td>{t.description}</td>
                                        </tr>
                                    ))}
                                </tbody>
