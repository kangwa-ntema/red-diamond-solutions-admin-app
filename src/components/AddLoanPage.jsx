...
const AddLoanPage = () => {
  ...
  const fetchEligibleClients = useCallback(async () => {
    ...
    if (!loanDataToRenew) {
      ...
      if (!token) {
        console.error(
          "AddLoanPage: No authentication token found. Redirecting to login."
        );
        clearAuthData();
        navigate("/loginForm");
        return;
      ...
      try {
        ...
        const clientsResponse = await fetch(
          `${BACKEND_URL}/api/clients?exclude_active_loan_clients=true`,
          {
            method: "GET",
        ...
        toast.error(
          `Error fetching clients: ${err.message || "Network error"}`
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      calculatedTotalRepaymentAmount = (
        loanAmount + parseFloat(calculatedInterestAmount)
      ).toFixed(2);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formData.loanTerm === null ||
      formData.loanTerm <= 0 ||
      !formData.startDate
    ) {
      setError(
        "Please fill in all required fields correctly (Loan Amount, Interest Rate, Loan Term must be positive numbers)."
      );
  ...
  return (
    <div className="addLoanPageContainer">
      <div className="addLoanPageContent">
        ...
        {successMessage && (
          <div className="addLoanSuccessMessage">{successMessage}</div>
        )}
        ...
        <form onSubmit={handleSubmit} className="addLoanForm">
          ...
          <div className="addLoanFormGroup">
            ...
            >
              ...
              {loanDataToRenew ? (
                <option
                  key={loanDataToRenew.client}
                  value={loanDataToRenew.client}
                >
          ...
          <div className="addLoanFormGroup">
            ...
            <input
              ...
              name="interestRate"
              value={
                formData.interestRate === null ? "" : formData.interestRate
              }
              onChange={handleChange}
              className="addLoanInput"
              step="0.01"
