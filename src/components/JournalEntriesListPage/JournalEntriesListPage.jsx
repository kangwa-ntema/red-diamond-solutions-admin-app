...
const JournalEntriesListPage = () => {
    ...
    const [filterRelatedDocumentType, setFilterRelatedDocumentType] = useState('');
    ...
    const fetchJournalEntries = useCallback(async () => {
        ...
        try {
            ...
            if (filterRelatedDocumentType) url += `type=${filterRelatedDocumentType}&`;
    ...
    }, [BACKEND_URL, navigate, filterStartDate, filterEndDate, filterAccountId, filterRelatedDocumentType]);
    ...
    const fetchAccountsForFilter = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch accounts for filter:', await response.json());
            }
        } catch (err) {
    ...
    if (loading && journalEntries.length === 0) {
        return <div className="journalEntryContainer loading">Loading journal entries...</div>;
    }
    ...
    return (
        <div className="journalEntryContainer">
            <Link to="/transactions" className="journalEntryBackLink">
                {"<"} Back to Main Dashboard
            </Link>
            <h1 className="journalEntryHeadline">Journal Entries</h1>

            <div className="journalEntryActions">
                <Link to="/journal-entries/add" className="addJournalEntryBtn">
                    + Create New Journal Entry
                </Link>
            </div>

            {/* Filter Section */}
            <div className="journalEntryFilters">
                <h3>Filter Journal Entries</h3>
                ...
                <div className="filterButtons">
                    <button onClick={handleApplyFilters} className="applyFiltersBtn">Apply Filters</button>
                    <button onClick={handleClearFilters} className="clearFiltersBtn">Clear Filters</button>
                </div>
            </div>

            {/* Journal Entries Table */}
            {journalEntries.length === 0 && !loading ? (
                <p className="noEntriesMessage">No journal entries found matching your criteria.</p>
            ) : (
                    ...
                    <table className="journalEntriesTable">
                        ...
                        <tbody>
                            {journalEntries.map(entry => (
                                <tr key={entry._id}>
                                    <td data-label="Entry No.">{entry.entryNumber || entry._id.substring(0, 8)}</td>
                                    <td data-label="Date">{new Date(entry.entryDate).toLocaleDateString()}</td>
                                    <td data-label="Description">{entry.description}</td>
                                    <td data-label="Related Document">
                                        {entry.relatedDocument?.type ? `${entry.relatedDocument.type} (ID: ${entry.relatedDocument.id?.substring(0,8)}...)` : 'N/A'}
                                    </td>
                                    <td data-label="Recorded By">{entry.recordedByUsername || 'N/A'}</td>
                                    <td data-label="Actions" className="journalEntryActionsCell">
                                        <Link to={`/journal-entries/${entry._id}`} className="viewJournalEntryBtn">View Details</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
