...
const CustomersDashboard = () => {
  ...
  const fetchCustomerData = useCallback(
    async (statusFilter = "all") => {
      ...
      if (!token) {
        console.error(
          "CustomersDashboard: No authentication token found. Redirecting to login."
        );
        clearAuthData();
        navigate("/login");
        return;
      ...
      try {
        ...
        if (response.status === 401 || response.status === 403) {
          console.error(
            "CustomersDashboard: Authentication expired or invalid. Logging out."
          );
          clearAuthData();
          navigate("/login");
          return;
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Failed to fetch customer data."
          );
        }
  ...
  const handleDelete = async (customerId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this customer? This will also affect their associated loans."
      )
    ) {
      const token = getToken();
      if (!token) {
  ...
  if (loading) {
    return (
      <div className="customersDashboardContainer">Loading customers...</div>
    );
  }

  if (error) {
    return (
      <div className="customersDashboardContainer" style={{ color: "red" }}>
        Error: {error}
      </div>
    );
  }
  ...
  return (
    <div className="customersDashboardContainer">
      <div className="customersDashboardHeading">

          <Link to="/dashboard">
            <button className="customersDashboardBackLink">
              Back to Main Dashboard
            </button>
          </Link>
          <h1 className="customersDashboardHeadline">CUSTOMERS OVERVIEW</h1>
            <h2 className="customerSummaryHeadline">SUMMARY</h2>
      </div>
      <div className="customersDashboard">
        <div className="customersDashboardPanel">
          ...
          <section className="customerSummarySection">
            <div className="customerSummaryCards">
              <div className="customerSummaryCard">
                <h3 className="customerSummaryCardTitle">Total Customers</h3>
                <p className="customerSummaryCardValue">
                  {overallSummary.totalCustomers}
                </p>
              </div>
              <div className="customerSummaryCard">
                <h3 className="customerSummaryCardTitle">Active Customers</h3>
                <p className="customerSummaryCardValue">
                  {overallSummary.activeCustomers}
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="customerActionBtns">
            <Link to="/customers/add">
              <button className="customerActionBtn newCustomerBtn">
                Add New Customer
              </button>
            </Link>
            <Link to="/loans/add">
              <button className="customerActionBtn newLoanBtn">Add Loan</button>
            </Link>
          </div>
        </div>
        <div className="customersDashboardContent">
          ...
          <div className="customerFilterBtns">
            <button
                    ...
                    {customers.length === 0 ? (
                        ...
                        <div className="customersTableContainer">
                            <table className="customersTable">
                                <thead lassName="customersTableHead">
                                    <tr>
                                        <th>ID</th>
                                        <th>Name</th>
                                        {/* <th>Email</th> */}
