...
const AddJournalEntryPage = () => {
  ...
  const totalCredits = lines
    ...
    .toFixed(2);
  const isBalanced =
    totalDebits === totalCredits && parseFloat(totalDebits) > 0; // Must be balanced and not all zero
  ...
  const fetchJournalEntry = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/journal-entries/${entryId}`,
        {
          method: "GET",
          headers: { Authorization: `Bearer ${token}` },
          credentials: "include",
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Failed to fetch journal entry details."
        );
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isBalanced) {
      toast.error(
        "Journal entry must be balanced (total debits equal total credits) and amounts cannot be zero."
      );
      setLoading(false);
      return;
    }
    ...
    if (
      ...
      toast.error(
        "All line items must have an account selected and a non-zero debit or credit amount."
      );
      setLoading(false);
      return;
    }
            ...
            <div className="balanceSummary">
              <p
                className={`totalDebits ${
                  totalDebits !== totalCredits ? "unbalanced" : ""
                }`}
              >
